...
G4double G4SQLoopProcessDiscr::PostStepGetPhysicalInteractionLength(const G4Track& track, G4double previousStepSize, G4ForceCondition* condition)
{
  *condition = NotForced; 
  ...
  G4Track * mytr = const_cast<G4Track *>(&track);
  if(sqrt(pow(mytr->GetPosition().rho(),2))>2.5*centimeter){//this is an important cut for the looping: if the radius of the particle is largher than 2.5cm its interaction length becomes 0 which means it will get killed
