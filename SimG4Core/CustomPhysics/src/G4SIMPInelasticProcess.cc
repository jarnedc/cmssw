...
float mS = aTrack.GetDynamicParticle()->GetMass();
float mS2 = pow(mS,2);
float mn = .94*GeV;
float mn2 = pow(mn,2);
float piS = aTrack.GetMomentum().mag();
float piSnew = piS - (mS2-mn2)*piS / (mS2+mn2+2*sqrt(mS2+pow(piS,2))*mn);

std::cout<< "%%% TR OLD: " << aTrack.GetKineticEnergy() << " " <<
aTrack.GetTotalEnergy() << 
  " " << mS << " " << piS << " " << piSnew << std::endl;

//aTrack.GetMomentum().setMag(piSnew);

std::cout<< "%%% TR NEW: " << aTrack.GetKineticEnergy() << " " <<
aTrack.GetTotalEnergy() << 
  " " << aTrack.GetDynamicParticle()->GetMass() << " " << aTrack.GetMomentum().mag() << " " << piSnew << std::endl;
